...
def apply_fft(im_path, im_size=64):
...
im_dir = '/media/evl/Public/Mahyar/Data/celeba/img_align_celeba_test/'
data_size = 100
im_size = 64
...
for fn in glob.glob(im_dir+'/*.jpg'):
	imf_proc, im_gray = apply_fft(fn, im_size)
...
imf_agg = np.mean(imf_data, axis=0)
plt.imshow(np.log(imf_agg))
plt.xlim(22, 42)
plt.ylim(22, 42)
plt.show()
